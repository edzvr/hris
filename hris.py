...
<style id="hris-global-sidebar-style">
    body.hris-sidebar-page {{ --hris-topbar-row-height: 48px; --hris-topbar-font-size: 17px; --hris-topbar-title-size: 24px; padding-left: 0 !important; padding-top: calc((var(--hris-topbar-row-height) * 2) + 28px) !important; }}
    #hris-global-sidebar {{ position: fixed; z-index: 10000; inset: 0 0 auto 0; width: 100% !important; max-width: 100vw; height: auto; box-sizing: border-box; display: grid; grid-auto-flow: column; grid-template-rows: repeat(2, var(--hris-topbar-row-height)); grid-auto-columns: max-content; align-items: stretch; gap: 8px 10px; padding: 10px 14px 12px; background: #122c3b; color: #f4f7f6; box-shadow: 0 3px 14px rgba(7,25,36,.28); font-family: Arial, sans-serif; overflow-x: auto; overflow-y: hidden; -webkit-overflow-scrolling: touch; overscroll-behavior-x: contain; touch-action: pan-x; }}
    #hris-global-sidebar h2 {{ grid-row: 1 / span 2; align-self: center; margin: 0 10px 0 0; font-size: var(--hris-topbar-title-size); line-height: 1.2; letter-spacing: .2px; white-space: nowrap; }}
    #hris-global-sidebar p {{ display: none; }}
    #hris-global-sidebar a {{ display: flex !important; align-items: center; justify-content: center; box-sizing: border-box; width: auto !important; height: var(--hris-topbar-row-height) !important; min-height: var(--hris-topbar-row-height) !important; min-width: max-content; padding: 12px 18px !important; margin: 0 !important; color: #f4f7f6; text-decoration: none; border-left: 0; border-bottom: 3px solid transparent; border-radius: 7px; font-size: var(--hris-topbar-font-size) !important; line-height: 1.25 !important; white-space: nowrap !important; overflow: hidden !important; text-overflow: ellipsis; }}
    ...
    @media (max-width: 760px), (max-device-width: 920px) {{ body.hris-sidebar-page {{ --hris-topbar-row-height: 42px; --hris-topbar-font-size: 15px; --hris-topbar-title-size: 18px; }} #hris-global-sidebar {{ gap: 6px 7px; padding: 8px 10px 10px; }} #hris-global-sidebar h2 {{ margin-right: 4px; }} #hris-global-sidebar a {{ padding: 10px 13px !important; }} }}
</style>
...
<script>(function() {{ const sizes={{small:['40px','14px','20px'],medium:['48px','17px','24px'],large:['56px','19px','28px']}}; const apply=()=>{{ const selected=localStorage.getItem('hrisTopbarSize')||'medium'; const size=sizes[selected]||sizes.medium; document.body.style.setProperty('--hris-topbar-row-height',size[0]); document.body.style.setProperty('--hris-topbar-font-size',size[1]); document.body.style.setProperty('--hris-topbar-title-size',size[2]); }}; apply(); window.addEventListener('storage',event=>{{ if(event.key==='hrisTopbarSize') apply(); }}); }})();</script>
'''
